...
void Compiler::createListingHeader()
{
	...
	listingFile << setw(9) << "STAG2:" << setw(34) << "Seokhee Han,  " << ctime(&nokoru)<< endl;
...
void Compiler::beginEndStmt() 
{
	...
	if (nextToken() != "." && token != ";")
		 processError("needs period or semicolon");
	if(token == ".")
		code("end",".");
}
...
{
	nextToken();
	while(isNonKeyId(token) || token == "read" || token == "write" ||  token == ";" ||  token == "begin"|| token == "if" || token == "while" ||token =="repeat")
	{
		execStmt();
	}
}
...
void Compiler::elsePt()
{
	...
	else if (token == "end" || token == ";" || token == "until" || token == "begin" || token == "while" || token == "if" || token == "repeat" || token == "read" || token == "write" || isNonKeyId(token)) 
	{
		code("post_if", popOperand());
	}
	else
		processError("unexpected token");
}
...
{
	if (s == "program" || s == "begin" || s == "end" || s == "var" || s == "const" || s == "integer" || s == "boolean" || s == "true" || s == "false" || s == "not" || s == "mod" || s == "div" || s == "and" || s == "or" || s == "read" || s == "write" ||  s == "if" || s == "then" || s == "else" || s == "repeat" || s == "while" || s == "do" || s == "until")
		return true;
	return false;
}
...
void Compiler::emitThenCode(string operand1, string operand2)
{
	...
	emit(" ","je", temp, "; if "+ operand1 + " is false then jump to end of if");
...
{
	if(operand2 != "")
		emit(" ","jmp", operand2, "; end if");
	emit(operand1 + ':', "","", "; end if");
	contentsOfAReg = "";
}
// emit code which follows end of 'if' statement
void Compiler::emitWhileCode(string , string operand2)
{
	string temp;
	temp = getLabel();
	emit(temp + ':', "","", "; while");
...
void Compiler::emitDoCode(string operand1, string operand2)
{
	...
	emit(" ","cmp", "eax,0", "; compare eax to 0");
	emit(" ","je", temp, "; if "+ operand1 + " is false then jump to end while");
	operandStk.push(temp);
	if(isTemporary(operand1))
		freeTemp();
	contentsOfAReg = "";
...
void Compiler::emitRepeatCode(string , string operand2)
{
	string temp;
	temp = getLabel();
	emit(temp+':', " ", " ", "; repeat" );
