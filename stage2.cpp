...
void Compiler::createListingHeader()
{
	...
	listingFile << left << "STAGE1:  Trevor Smith, Seokhee Han\t" << ctime(&now) << endl;
	listingFile << "LINE NO." << setw(30) << right << "SOURCE STATEMENT" << endl << endl;
}
...
void Compiler::term()     
{
	if (!(token == "not" || token == "true" || token == "false" || token == "-" || token == "+" || token == "(" || isInteger(token) || isNonKeyId(token) || token == "mod"))
   {
		processError("incorrect term usage");
   }
...
void Compiler::whileStmt()
{
   ...
   string hold1 = popOperand();
	string hold2 = popOperand();
	code("post_while", hold1, hold2);
}
...
void Compiler::emitPrologue(string progName, string operand2) 
{
	...
	objectFile << "; Trevor Smith, Seokhee Han\t" << ctime(&now);
...
void Compiler::emitStorage() 
{
	...
	for (it = symbolTable.begin(); it != symbolTable.end(); it++)
	{
		if(it->second.getInternalName().at(0) == 'B'&& it->second.getValue() == "false" ) {
			emit(it->second.getInternalName(), "dd", "0", "; false");
      }
		else if(it->second.getInternalName().at(0) == 'B' && it->second.getValue() == "true" ) {
			emit(it->second.getInternalName(), "dd", "-1", "; true");
      }
...
{
	return s == "program" || s == "begin" || s == "end" || s == "var" || s == "const" || s == "integer" || s == "boolean" || s == "true" || s == "false" || s == "not" || s == "mod" || s == "div" || s == "and" || s == "or" || s == "read" || s == "write" ||  s == "if" || s == "then" || s == "else" || s == "repeat" || s == "while" || s == "do" || s == "until";
}
